...
const Books = ({ books }) => {
  ...
  useEffect(() => {  
    if (subscriptionData?.bookAdded) {  
      setFilteredBooks((prevBooks) => [...prevBooks, subscriptionData.bookAdded]);  
    }  
    else if (!loading && data) {  
      setFilteredBooks(data.allBooks);  
    }  
  }, [subscriptionData, loading, data]);  
  
  const genres = filteredBooks.reduce((allGenres, book) => {  
    return allGenres.concat(book.genres);  
  }, []);  
